...
| Review: primera captura de validación falla con `unknown field "inspections"` | **Defecto del harness (#33), fuera de este repo.** Reintentar tras STATUS fresco: el mismo vector suele aprobar al segundo envío. Criterio: una captura que devuelva veredicto en el primer intento. |
| Review: START falla con `candidate-owner-preparation-failed` y hay un `REVIEW-MAINTENANCE.lock` huérfano | **Defecto del harness (#45), fuera de este repo.** Si ningún proceso retiene el lock (`ls -la .git/gentle-ai/` y fecha vieja), moverlo a un lado (`mv .git/gentle-ai/REVIEW-MAINTENANCE.lock /tmp/`) y reintentar START. No borrar a ciegas: si el lock es legítimo tiene vida corta. |
