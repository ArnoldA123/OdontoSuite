...
import Page from '../../../resources/js/modules/business-intelligence/BusinessIntelligencePage.vue'
const router = createRouter({
  history: createMemoryHistory(),
  routes: [{ path: '/:pathMatch(.*)*', component: { template: '<div />' } }]
})
describe('business-intelligence smoke', () => {
  it('mounts BusinessIntelligencePage.vue and renders content', async () => {
    const wrapper = mount(Page, { global: { plugins: [router, uiComponents] } })
    await flushPromises()
    expect(wrapper.text()).toContain('Business Intelligence')
    wrapper.unmount()
  })
})
