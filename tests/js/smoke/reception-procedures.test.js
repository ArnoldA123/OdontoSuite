...
import Page from '../../../resources/js/modules/reception-procedures/ReceptionProceduresPage.vue'
const router = createRouter({
  history: createMemoryHistory(),
  routes: [{ path: '/:pathMatch(.*)*', component: { template: '<div />' } }]
})
describe('reception-procedures smoke', () => {
  it('mounts ReceptionProceduresPage.vue and renders content', async () => {
    const wrapper = mount(Page, { global: { plugins: [router, uiComponents] } })
    await flushPromises()
    expect(wrapper.text()).toContain('Catálogo de Procedimientos')
    wrapper.unmount()
  })
})
