...
import Page from '../../../resources/js/modules/medical-records/MedicalRecordsPage.vue'
const router = createRouter({
  history: createMemoryHistory(),
  routes: [{ path: '/:pathMatch(.*)*', component: { template: '<div />' } }]
})
describe('medical-records smoke', () => {
  it('mounts MedicalRecordsPage.vue and renders content', async () => {
    const wrapper = mount(Page, { global: { plugins: [router, uiComponents] } })
    await flushPromises()
    expect(wrapper.text()).toContain('Historias Clínicas')
    wrapper.unmount()
  })
})
