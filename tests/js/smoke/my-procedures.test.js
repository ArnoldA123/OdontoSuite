...
import Page from '../../../resources/js/modules/my-procedures/MyProceduresPage.vue'
const router = createRouter({
  history: createMemoryHistory(),
  routes: [{ path: '/:pathMatch(.*)*', component: { template: '<div />' } }]
})
describe('my-procedures smoke', () => {
  it('mounts MyProceduresPage.vue and renders content', async () => {
    const wrapper = mount(Page, { global: { plugins: [router, uiComponents] } })
    await flushPromises()
    expect(wrapper.text()).toContain('Mis Procedimientos')
    wrapper.unmount()
  })
})
