<template>
  <div class="space-y-3">
    ...
    <template v-else>
      <div class="relative">
        <input v-model="search" type="text" placeholder="Buscar por nombre o código..." class="w-full px-3 py-2 pl-9 border border-theme rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-accent bg-theme-surface-elevated text-theme-primary">
      ...
      <div class="max-h-72 overflow-y-auto border border-theme rounded-lg divide-y divide-theme">
        <button v-for="proc in filteredProcedures" :key="proc.id" type="button" class="w-full px-3 py-2 text-left hover:bg-theme-surface transition-colors flex items-start justify-between gap-2" :class="modelValue === proc.id ? 'bg-accent text-white hover:bg-accent' : ''" @click="select(proc)">
        ...
        <div v-if="!filteredProcedures.length" class="px-3 py-4 text-center text-sm text-theme-secondary">
          Sin resultados
        </div>
      </div>

      <div v-if="totalPages > 1" class="flex justify-center pt-2">
        <UiPagination :current-page="currentPage" :total-pages="totalPages" :total="pagination.total" :per-page="pagination.per_page" @page-change="onPageChange" />
      </div>
    </template>
  </div>
</template>
