<template>
  <div class="space-y-3">
    ...
    <div :class="horizontal ? 'flex gap-3' : 'space-y-2'">
      <label v-for="option in options" :key="option.value" :class="optionClasses(option.value)">
        <input
          type="radio"
          :name="name"
          :value="option.value"
...
const optionClasses = value => {
  const base =
    'flex-1 px-4 py-3 rounded-xl border-2 cursor-pointer smooth-transition text-center font-medium'
  const selected =
