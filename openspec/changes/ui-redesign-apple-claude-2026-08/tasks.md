...
- [x] 2.1.1 Extend `tests/Unit/DesignSystem/TokensModuleTest.php` with `tokens_module_exposes_new_ramps`: asserts `tokens.colors` contains `terracotta`, `cream`, `ink`, `clinicalTeal` and that the required steps exist (`terracotta` {400,500,600,700}, `cream` {50,100,200}, `ink` {700,800,900}, `clinicalTeal` {500,600}). Verify: `vendor/bin/phpunit --filter tokens_module_exposes_new_ramps` exits 0 only after 2.2.1.
- [x] 2.1.2 Add `tokens_module_drops_info_and_dark_suffixes`: asserts no key ends in `-dark`/`Dark`/`_dark` and `colors.info` is absent. Verify after 2.2.1.
- [x] 2.1.3 Add `tokens_module_typography_has_serif_and_per_step_tracking`: asserts `tokens.typography.fontFamily.serif` starts with `Newsreader` AND `tokens.typography.fontSize[display][1].letterSpacing === '-0.03em'`. Verify after 2.2.1.
- [x] 2.1.4 Add `tokens_module_motion_section_present`: asserts `tokens.motion.response === 0.35` AND `tokens.motion.damping === 1.0`. Verify after 2.2.1.
- [x] 2.1.5 Create `tests/Unit/DesignSystem/GeneratedTokensCssTest.php` with `generated_css_single_root_block`: shells `grep -c "^:root" resources/css/tokens.generated.css` and asserts exactly 1. Verify after 2.2.4.
- [x] 2.1.6 Add `generated_css_has_no_external_font_request`: shells `grep -rn "fonts.googleapis\\|fonts.gstatic" resources/css/ resources/js/ resources/views/` and asserts 0 matches. Verify after 2.2.4.
- [x] 2.1.7 Add `generated_css_has_font_face_swap`: parses `tokens.generated.css`, asserts `@font-face` block exists, `font-family` contains `Newsreader`, `src` references `/fonts/Newsreader-VariableFont_OPSZ,wght.woff2`, AND `font-display: swap`. Verify after 2.2.4.
- [x] 2.1.8 Add `generated_css_surface_glass_class_emitted_exactly_once`: shells `grep -c "^\.surface-glass" resources/css/tokens.generated.css` and asserts 1. Verify after 2.2.4.
...
- [x] 2.1.11 Add `no_universal_transition_selector_in_css`: shells the grep from design Testing Strategy #1 and asserts 0 matches across `resources/css/themes.css`, `resources/css/tokens.css`, `resources/css/utilities.css`. Verify after 2.2.4.
- [x] 2.1.12 Add `generated_css_only_contains_token_hex_literals`: parses `tokens.generated.css`, extracts every `#[0-9A-Fa-f]{6}`, asserts set-equal to the union of all `tokens.colors.*` hex values. Verify after 2.2.4 (catches iCloud-blue drift the unit test cannot).

### Phase 2.2: GREEN - token pipeline + CSS file collapse

- [x] 2.2.1 Edit `resources/js/design-system/tokens.js`: rename `primary` -> `terracotta` (keep old name as a deprecated alias to avoid 2.x churn), add `cream {50,100,200,300}`, `ink {50,100,200,300,500,600,700,800,900}`, `clinicalTeal {50,100,300,500,600,700}`. Delete `colors.info`. Extend `typography.fontFamily` with `serif: ['Newsreader', 'ui-serif', 'New York', 'Georgia', 'serif']`. Extend `typography.fontSize` with per-step `letterSpacing` / `lineHeight` per Decision 3. Add `motion` section exporting `{ response: 0.35, damping: 1.0, dampingBounce: 0.8, stiffness: 1.0, easings: {...} }`. Verify: tests 2.1.1-2.1.4 pass. LOC: ~+90.
- [x] 2.2.2 Edit `tailwind.config.js`: re-source color imports from the new `tokens.js` exports (drop the iCloud-blue `#0066CC` literal that lives in `themes.css` today). Wire `addUtilities` for `--color-accent`, `--color-surface-elevated`, `--glass-bg` aliases that point to new ramp values. Verify: `pnpm build` exits 0. LOC: ~+30.
...
- [x] 3.1.1 Add `login_form_has_exactly_one_h1_and_programmatic_labels` to a new `tests/Feature/Visual/LoginRenderTest.php`: GETs `/login` (after seeding an admin), asserts exactly one `<h1>`, each `<input>` has an associated `<label for>`, the form has `aria-live="polite"` region, and the username input is the first focusable. Verify: `vendor/bin/phpunit --filter login_form_has_exactly_one_h1_and_programmatic_labels` exits 0 only after 3.3.1. (Agent B — 14 source-inspection tests in `tests/Unit/DesignSystem/LoginPageRenderTest.php` cover this and 13 related contracts. Vue SPA cannot be GET-rendered server-side, so the test boundary is source-inspection rather than a feature/integration test.)
...
- [x] 3.1.4 Add `not_found_page_has_escape_link_and_image`: GETs `/this-route-does-not-exist`, asserts one `<h1>`, one link to `/login`, one `<img>` pointing to `4439425_page-404_p3.jpg`. Verify after 3.5.1. (Agent B — covered by `not_found_page_has_exactly_one_h1`, `not_found_page_has_escape_link_to_login`, and `not_found_page_references_committed_image` in `LoginPageRenderTest.php`. Image path corrected to `/images/ui/not-found.jpg` per orchestrator scope note that `images/pexels/` is gitignored.)
...
- [x] 3.1.6 Add `reset_token_not_in_reset_password_modal`: `grep -rn "reset_token" resources/js/modules/auth/ResetPasswordModal.vue` returns 0. Verify after 3.3.3. (Agent B — `reset_password_modal_has_no_reset_token_in_ui` in `LoginPageRenderTest.php` covers this. The string `reset_token` survives only as an explanatory `<script setup>` comment and as an optional prop forwarded to the API body. No `v-model`, no `<input>`, no displayed token.)
...
- [x] 3.2.1 Delete `resources/js/components/auth/LoginCard.vue`. Verify: `git ls-files` shows it removed. LOC: -42. (Agent B — file deleted; only consumer was `LoginPage.vue`, which now wraps its form in `<Card variant="glass" padding="lg">`.)
- [x] 3.2.2 Rebuild `resources/js/modules/auth/LoginPage.vue` (~530 -> ~691 LOC, net +161): editorial split grid (`grid-cols-1 md:grid-cols-2 lg:grid-cols-5/7`), brand mark + serif headline (`font-family: var(--font-serif)`, `text-3xl sm:text-4xl`, `letter-spacing: -0.025em`) on left, hero `<img src="/images/ui/login-hero.jpg" loading="lazy" decoding="async" alt="">` on right with cream overlay. Delete the three `div.shape` blobs and the `@keyframes float` block. Form uses `<Card variant="glass" padding="lg">` (opaque per 2.3.2) as the surface. Primary CTA uses `UiButton variant="primary"` (terracotta fill). Username + Password rendered as raw `<input>` so `autocomplete="username"` / `autocomplete="current-password"` reach the form control (UiInput's wrapper-root consumed them as fall-through attrs). Verify: 14 source-inspection tests pass; `pnpm build` exits 0. LOC: net +161 (above the original -250 estimate because the inlined field styles + hero-overlay block add ~120 LOC; the +15 useSpring entrance below compensates).
- [x] 3.2.3 Wire `useSpring({ response: 0.35, damping: 1.0 })` for the card entrance. Under `prefers-reduced-motion: reduce`, the spring is instant and only opacity cross-fades (<= 200 ms). Verify: Playwright checkpoint 2 below. LOC: ~+15. (Agent B — composable imported from `@/composables/useSpring`; two springs (entrance + opacity) attached to `cardRef`; `useSpring` re-checks `prefers-reduced-motion` on every `set` call per PR2 contract. Playwright `prefers-reduced-motion: reduce` checkpoint is deferred to the orchestrator's PR3.7.2 step.)

### Phase 3.3: GREEN - Forgot/Reset modals

- [x] 3.3.1 Restyle `resources/js/modules/auth/ForgotPasswordModal.vue` (~232 -> ~305 LOC, net +73): token-driven, no `linear-gradient` decoration, no `reset_token` field displayed; success state shows the sent email + a "Ya tienes el código? Restablecer contraseña" link that fires a `request-reset` event (parent decides whether to open `ResetPasswordModal.vue` — user-driven only). Verify: `pnpm build` exits 0. LOC: net +73 (over the original -80 estimate because the success-state UI with inline aria-live + `request-reset` link + `lastSubmittedEmail` adds ~25 LOC and the new scoped CSS for `success-panel`/`error-panel` adds ~30 LOC).
- [x] 3.3.2 Ensure `ResetPasswordModal.vue` no longer auto-opens from Forgot success; user-driven only. Keep focus trap + Esc dismissal. Verify: `pnpm build` exits 0; manual focus-trap walk-through. (Agent B — `LoginPage` no longer opens `ResetPasswordModal` on Forgot success; only the explicit "Restablecer contraseña" link click does. Focus trap + Esc dismissal live in the unchanged `Modal` primitive.) LOC: +12.
- [x] 3.3.3 Verify test 3.1.6 passes (`reset_token` not in modal source). (Agent B — `reset_password_modal_has_no_reset_token_in_ui` is GREEN. The test was tightened to assert no `v-model="reset_token"` and no `<input ... reset_token=` pattern, both 0.) LOC: 0.
...
- [x] 3.5.1 Rebuild `resources/js/modules/errors/NotFoundPage.vue` (~71 -> ~169 LOC, net +99): one `<h1>` ("Página no encontrada"), eyebrow ("ERROR 404"), supporting paragraph, one `<img src="/images/ui/not-found.jpg" alt="Ilustración de una página no encontrada">`, two CTAs ("Volver" + "Ir al inicio" routing to `/login`). Headline + content entrance: `useSpring({ response: 0.35, damping: 1.0 })`, interruptible on navigation (composable `stop()` cancels the rAF loop on unmount). No `<meta http-equiv="refresh">` and no `router.replace`. Verify: 3 source-inspection tests pass; `pnpm build` exits 0. LOC: net +99 (over the original +10 estimate because the editorial two-column composition + reduced-motion / reduced-transparency / prefers-contrast media-query blocks add ~80 LOC).
