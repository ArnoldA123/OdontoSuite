...
- [x] Local `php artisan test --filter=SddCheckMigrationsTest` passes (pure-string scan, runs on SQLite in-memory with `DB_CONNECTION=sqlite` and `DB_DATABASE=:memory:` pinning).
